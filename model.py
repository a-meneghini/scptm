...
class SCPTM:
    ...
    def fit(
        ...
        self._ctx_embs_list = [
            ctx.to("cpu") if ctx is not None else None
            for ctx in [
                collect_contextual_embeddings(
    ...
    def fit_transform(
        ...
        **fit_kwargs,
        ...
        self.fit(source, source_type=source_type, text_col=text_col, **fit_kwargs)
        return self._theta
    ...
    def get_topic_info(self, top_k: int = 10) -> pd.DataFrame:
        """
        Return a DataFrame with topic index, top words, and dominant doc count.
        """
        self._check_fitted()
        top_words = extract_top_words(self._nn, self._vocab, top_k=top_k)
    ...
    def get_topics_dict(self, top_k: int = 5) -> dict:
        """Return separated single-word + phrase topic keywords."""
        self._check_fitted()
        if self._topics_dict is None:
    ...
    def load(cls, path: Union[str, Path]) -> "SCPTM":
        ...
        # Rebuild minimal graph data for transform()
        from torch_geometric.data import HeteroData
        gd = HeteroData()
        gd["doc"].x  = state["graph_doc_x"].to(model._device)
        gd["word"].x = state["graph_word_x"].to(model._device)
        gd["doc", "contains", "word"].edge_index   = torch.empty((2,0), dtype=torch.long)
        gd["word", "rev_contains", "doc"].edge_index = torch.empty((2,0), dtype=torch.long)
        gd["word", "relates", "word"].edge_index   = torch.empty((2,0), dtype=torch.long)
